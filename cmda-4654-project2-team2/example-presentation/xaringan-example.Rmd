...
**Main Idea:** Gradient boost works by creating small trees (known as weak learners) of a set size, fitting the data and testing it, and creating new trees to fit the residuals of the previous tree.

--

Each step of the algorithm creates a new tree that may be small or large, depending on your tuning parameters. The next step in the algorithm creates a new tree that predicts the errors (residuals) from the last tree. All of the trees are then **scaled** by a tuning parameter between 0 and 1. *This scale tuning parameter basically determines how quickly our boosted trees find a good fit, AKA how many trees need to be created to minimize the errors.*
...
You can see how this is similar to random forests
...
```{r cars, fig.height=4, fig.width=7}
par(mar = c(4, 4, 1, .1))
